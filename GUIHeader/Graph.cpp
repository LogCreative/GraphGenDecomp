...
Progressbar::Progressbar(Point _xy, int _w, int _h, Color _fC) :
    xy(_xy), w(_w), h(_h), fill_Color(_fC), p(0) {
    draw_lines();
}
...
void Progressbar::draw_lines() const {
    Rectangle* border_rect = new Rectangle(xy, w, h);
    border_rect->set_color(Color::black);
    
    Rectangle* fill_rect = new Rectangle(xy, w * p, h);
    fill_rect->set_fill_color(fill_Color);
    fill_rect->draw_lines();
    border_rect->draw_lines();
}
